...
public:
  ...
  // ---------------------------------------------------
  // c'tor & d'tor
  explicit StopWatch (std::wostream& = std::wcout, const wchar_t* = L"StopWatch");
  ...
  auto loggingMessage () const noexcept -> const wchar_t* const;
  template <typename Duration = duration>
...
private:
  ...
  const wchar_t* logMessage    {L"StopWatch"};
  std::wostream& loggingStream {std::wcout};
};
...
inline StopWatch<ClockT>::StopWatch(std::wostream& loggingStrm, const wchar_t* logMsg)
  : logMessage    {logMsg}
  , loggingStream {loggingStrm}
{}
 ...
 */
template <typename ClockT>
inline auto StopWatch<ClockT>::loggingMessage() const noexcept -> const wchar_t* const
{
  return logMessage;
}
...
{
  const auto oldPrecision = loggingStream.precision();
  const auto currentFlags = loggingStream.flags();

  loggingStream << logMessage << L" : " << std::setprecision(precision) << std::fixed
                << getDuration<Duration>().count() << DurationString<Duration, wchar_t>::value << L'\n';

  loggingStream.setf(currentFlags);
  loggingStream.precision(oldPrecision);
}
