 ...
 */
template <typename T>
inline void multiplyArray(T* x, std::size_t const size, T const& factor)
{
...
inline void multiplyArray(T* x, std::size_t const size, S const& factor)
{
  for(const auto end = x + size; x != end; ++x)
  {
    *x *= factor;
...
void multiplyArray(T (&values)[N], T const& factor)
{
...
void multiplyArray(T (&values)[N], S const& factor)
{
  std::transform(/*std::execution::par, */ std::begin(values), std::end(values), std::begin(values),
                 [&factor](T const& v) { return v * factor; });
}
